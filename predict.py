...
def process_image(image_path):
    ...
    #image = cv2.resize(image, input_size)  # Resize the image
...
model = SimpleObjectDetector(num_boxes=80, num_classes=37).to(device)
model.load_state_dict(torch.load('best.pth'))  # Load the saved weights
model.eval()  # Set the model to evaluation mode

# Process the image
image_path = 'images/7fB0x.jpg'  # Replace with your image path
...
def draw_boxes(image, boxes, labels, confidences, label_names, threshold=0.5):
    for box, label, confidence in zip(boxes, labels, confidences):
        ...
        print(box.shape)
        x_center, y_center, width, height = 1920*box[0], 0*box[1], 20*box[2], 20*box[3]
